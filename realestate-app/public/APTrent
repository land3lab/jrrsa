// 국토교통부_아파트 전월세 실거래자료 (RTMSDataSvcAptRent)
const BASE = "http://apis.data.go.kr/1613000/RTMSDataSvcAptRent/getRTMSDataSvcAptRent";
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const num = (v) => String(v == null ? "" : v).replace(/,/g, "").trim();
    let items = result.items.map((it) => ({
      ...
      보증금: num(it.deposit || it.보증금),
      월세: num(it.monthlyRent || it.월세) || "0",
      계약구분: (it.contractType || "").trim(),
      계약기간: (it.contractTerm || "").trim(),
      종전보증금: num(it.preDeposit),
      종전월세: num(it.preMonthlyRent),
      년: it.dealYear || it.년,
      월: it.dealMonth || it.월,
      일: it.dealDay || it.일,
    }));
